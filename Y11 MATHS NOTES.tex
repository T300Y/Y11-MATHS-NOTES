...
\thm{Odd and Even functions}{
    ...
    \newline
    A function/relation  is called odd if the point of symmetry is at the origin, this means that if rotated $180\deg$ the graph remains unchanged
    \newline
...
\thm{Cubic Polynomials}{
    Cubic polymials are polynomials of the third degree, a cubic function has 1 y intercept but upto 3 x intercepts
    \begin{center}
        \includegraphics[scale=0.25]{hyperbolicfunction1.png}
    \end{center}
    -    in $f(x) = x^{3} + 1$ the constant 1 is the y intercept
    \newline
    If ther form is $kx^{3}$ when $k>0$ it is a increasing function and when $k<0$ it is a decreassing function.
    The point of inflection in the $x^{3}$ function is wear the gradient of the line changes
    \newline
    To find the point of inflexion we can get the equation in the form $f(x) = k(x-b)^{3}+6$ which is the same as $kx^{3}$ however the inflexion point is $(b,c)$
    \newline
...
\thm{Hyperbolic Function}{
    A hyperbola occurs in the form of $\frac{k}{x}$ where k is a constant. This equation tells us that when oine variable increases the other decreases..
    \newline
    A hyperbola is a discontinuous function meaning there are gaps. The are two asymptotes along the x and y axis. This means that x cannot equal 0 and y cannot equal 0.
    \begin{center}
        \includegraphics[scale=0.5]{hyperbolafunction1.png}
    \end{center}
    To find the y intercept set x = 0 and solve for y
    \newline
    To find the x asymptote(Domain)  solve expression on the denominator for x.
    \newline
    To find the y asymptote(Range)  substitute the x asymtote into the denominator expression and solve for y

        

}

\qs{}{\begin{center}  Find x and y asymptotes and y intercept of the following equation $f(x)=\frac{1}{x+1}$  \end{center}}
$$x+1\neq 0$$
$$x \neq -1$$
$$y \neq x +1$$
$$y \neq -1+1$$
...
$$y int = \frac{1}{1}$$
\begin{center}
    $\therefore$ y asymptote = 0, x asy,mptote = -1 and y intercept = 1
\end{center}
\qs{}{\begin{center}  Find y and x asymptotes and the y intercept of the function $f(x) = \frac{4}{x-1}$\end{center}}
$$x -1 \neq 0$$
$$x \neq 1$$
$$y \neq x-1$$
$$y \neq 1-1$$
...
$\therefore$ y asymptote = 0, x asy,mptote = 1 and y intercept = -4
\end{center}
\end{document}
